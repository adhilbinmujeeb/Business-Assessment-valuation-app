...
try:
    client = MongoClient(MONGO_URI)
    db = client.business_assessment
    questions_collection = db.questions
except Exception as e:
    st.error(f"Failed to connect to MongoDB: {str(e)}")

def get_initial_questions():
    """Get the initial set of general business questions"""
    try:
        initial_questions = list(questions_collection.find({
            "category": "Core Business Analysis Questions",
            "is_general": True
        }).limit(5))
        return initial_questions
    except Exception as e:
        st.error(f"Error fetching initial questions: {str(e)}")
        return []

def analyze_business_type(responses):
    """Analyze responses to determine business type and key areas"""
    # Extract business type and key focus areas from initial responses
    business_type = None
    key_areas = []
    
    for qa in responses:
        if "type of business" in qa["question"].lower():
            business_type = qa["answer"]
        if "main challenges" in qa["question"].lower():
            key_areas.extend([area.strip() for area in qa["answer"].split(",")])
            
    return business_type, key_areas

def get_targeted_questions(business_type, key_areas, asked_questions):
    """Get questions specific to business type and key areas"""
    try:
        targeted_questions = list(questions_collection.find({
            "business_type": business_type,
            "category": {"$in": key_areas},
            "question": {"$nin": asked_questions}
        }).limit(3))
        return targeted_questions
    except Exception as e:
        st.error(f"Error fetching targeted questions: {str(e)}")
        return []

def assess_response_quality(responses):
    """Assess if we have enough quality information for a report"""
    # Check number of responses
    if len(responses) < 8:
        return False
        
    # Check response lengths
    short_responses = sum(1 for qa in responses if len(qa["answer"]) < 50)
    if short_responses > len(responses) / 3:
        return False
        
    # Check coverage of key areas
    covered_categories = set(qa["category"] for qa in responses)
    if len(covered_categories) < 3:
        return False
        
    return True

def generate_business_report(responses, business_type):
    """Generate a comprehensive business report"""
    report_prompt = f"""
    As an expert business analyst, generate a comprehensive business report based on the following Q&A session for a {business_type} business:

    {format_qa_for_prompt(responses)}
...
def format_qa_for_prompt(responses):
    """Format Q&A pairs for the AI prompt"""
    return "\n\n".join([
...
if "conversation_history" not in st.session_state:
    ...
    st.session_state.key_areas = None

if not st.session_state.assessment_completed:
    if not st.session_state.conversation_history:
        # Start with initial questions
        initial_questions = get_initial_questions()
        if initial_questions:
            st.session_state.current_question = initial_questions[0]
    
    if st.session_state.current_question:
        st.write(st.session_state.current_question["question"])
        user_answer = st.text_area("Your answer:", key="current_answer")
        ...
        if st.button("Submit Answer"):
            if user_answer:
                ...
                # After initial questions, determine business type and key areas
                if len(st.session_state.conversation_history) == 5:
                    st.session_state.business_type, st.session_state.key_areas = analyze_business_type(st.session_state.conversation_history)
                
                # Get next question
                asked_questions = [qa["question"] for qa in st.session_state.conversation_history]
                
                if st.session_state.business_type:
                    # Get targeted questions based on business type
                    next_question = get_targeted_questions(
                        st.session_state.business_type,
                        st.session_state.key_areas,
                        asked_questions
                    )
                    if next_question:
                        st.session_state.current_question = next_question[0]
                    else:
                        # Generate AI question if no suitable questions in database
                        st.session_state.current_question = generate_context_aware_question(
                            st.session_state.conversation_history,
                            st.session_state.business_type
                        )
                
                # Check if we have enough information
                if assess_response_quality(st.session_state.conversation_history):
                    st.session_state.assessment_completed = True
                    st.rerun()

if st.session_state.assessment_completed:
    st.success("Assessment completed! Generating your business report...")
